...
public class FalconPathPlanner {
	...
	/**
	 * Constructor, takes a Path of Way Points defined as a double array of column
	 * vectors representing the global cartesian points of the path in {x,y}
	 * coordinates. The waypoint are traveled from one point to the next in
	 * sequence.
	 * 
	 * For example: here is a properly formated waypoint array
	 * 
	 * double[][] waypointPath = new double[][]{ {1, 1}, {5, 1}, {9, 12}, {12, 9},
	 * {15,6}, {15, 4} };
	 * 
	 * This path goes from {1,1} -> {5,1} -> {9,12} -> {12, 9} -> {15,6} -> {15,4}
	 * 
	 * The units of these coordinates are position units assumed by the user (i.e
	 * inch, foot, meters)
	 * 
	 * @param path
	 */
	...
	/**
	 * Prints Cartesian Coordinates to the System Output as Column Vectors in the
	 * Form X Y
	 * 
	 * @param path
	 */
	...
	/**
	 * Performs a deep copy of a 2 Dimensional Array looping thorough each element
	 * in the 2D array
	 * 
	 * BigO: Order N x M
	 * 
	 ...
	 * 
	 * BigO: Order N + Order M, Where N is length of original Path, and M is length
	 * of Nodes found in Path
	 * 
	 * @param path
	 * @return
	 */
	...
	/**
	 * Returns Velocity as a double array. The First Column vector is time, based on
	 * the time step, the second vector is the velocity magnitude.
	 * 
	 * BigO: order N
	 * 
	 * @param smoothPath
	...
	/**
	 * optimize velocity by minimizing the error distance at the end of travel when
	 * this function converges, the fixed velocity vector will be smooth, start and
	 * end with 0 velocity, and travel the same final distance as the original
	...
	double[][] velocityFix(double[][] smoothVelocity, double[][] origVelocity, double tolerance) {
		...
		/*
		 * pseudo 1. Find Error Between Original Velocity and Smooth Velocity 2. Keep
		 * increasing the velocity between the first and last node of the smooth
		 * Velocity by a small amount 3. Recalculate the difference, stop if threshold
		 * is met or repeat step 2 until the final threshold is met. 3. Return the
		 * updated smoothVelocity
		 */
	...
	/**
	 * This method calculates the integral of the Smooth Velocity term and compares
	 * it to the Integral of the original velocity term. In essence we are comparing
	 * the total distance by the original velocity path and the smooth velocity path
	 * to ensure that as we modify the smooth Velocity it still covers the same
	 * distance as was intended by the original velocity path.
	 * 
	 * BigO: Order N
	 * 
	 * @param origVelocity
	...
	/**
	 * This method calculates the optimal parameters for determining what amount of
	 * nodes to inject into the path to meet the time restraint. This approach uses
	 * an iterative process to inject and smooth, yielding more desirable results
	 * for the final smooth path.
	 * 
	 * Big O: Constant Time
	 * 
	...
	public void leftRight(double[][] smoothPath, double robotTrackWidth) {
		...
		for (int i = 0; i < gradient.length; i++) {
			...
			// convert to degrees 0 to 360 where 0 degrees is +X - axis, accumulated to
			// aline with WPI sensor
	...
	/**
	 * This code will calculate a smooth path based on the program parameters. If
	 * the user doesn't set any parameters, the will use the defaults optimized for
	 * most cases. The results will be saved into the corresponding class members.
	 * The user can then access .smoothPath, .leftPath, .rightPath,
	 * .smoothCenterVelocity, .smoothRightVelocity, .smoothLeftVelocity as needed.
	 * 
	 * After calling this method, the user only needs to pass
	 * .smoothRightVelocity[1], .smoothLeftVelocity[1] to the corresponding speed
	 * controllers on the Robot, and step through each setPoint.
	 * 
	 * @param totalTime
	 *            - time the user wishes to complete the path in seconds. (this is
	 *            the maximum amount of time the robot is allowed to take to
	 *            traverse the path.)
	 * @param timeStep
	 *            - the frequency at which the robot controller is running on the
	 *            robot.
	 * @param robotTrackWidth
	...
	public void calculate(double totalTime, double timeStep, double robotTrackWidth) {
		 ...
		 * 2. Calculate how many total datapoints we need to satisfy the controller for
		 * "playback" 3. Simultaneously inject and smooth the path until we end up with
		 * a smooth path with required number of datapoints, and which follows the
		 * waypoint path. 4. Calculate left and right wheel paths by calculating
		 * parallel points at each datapoint
		 */
