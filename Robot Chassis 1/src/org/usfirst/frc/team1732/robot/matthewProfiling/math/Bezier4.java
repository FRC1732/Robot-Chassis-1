	...
	public Vector at(double t) {
		return new Vector(
				...
				points[0].getY() * t * t * t + 3 * points[1].getY() * t * t * (1 - t)
						+ 3 * points[2].getY() * t * (1 - t) * (1 - t)
						+ points[3].getY() * (1 - t) * (1 - t) * (1 - t));
	}

	@Override
	public Vector dAt(double t) {
		return new Vector(
				derivative[0].getX() * t * t + 2 * derivative[1].getX() * t * (1 - t)
						+ derivative[2].getX() * (1 - t) * (1 - t),
				derivative[0].getY() * t * t + 2 * derivative[1].getY() * t * (1 - t)
						+ derivative[2].getY() * (1 - t) * (1 - t));
	}
