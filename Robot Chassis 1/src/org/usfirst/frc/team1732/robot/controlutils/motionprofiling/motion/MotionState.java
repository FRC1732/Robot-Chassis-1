...
public class MotionState {
	...
	public double nextTimeAtPos(double pos) {
		if (MotionUtil.epsilonEquals(pos, this.pos, MotionUtil.kEpsilon)) {
			// Already at pos.
			return t;
		}
		if (MotionUtil.epsilonEquals(acc, 0.0, MotionUtil.kEpsilon)) {
			// Zero acceleration case.
			final double delta_pos = pos - this.pos;
			if (!MotionUtil.epsilonEquals(vel, 0.0, MotionUtil.kEpsilon)
					&& Math.signum(delta_pos) == Math.signum(vel)) {
	...
	public boolean equals(MotionState other, double epsilon) {
		return coincident(other, epsilon) && MotionUtil.epsilonEquals(acc, other.acc, epsilon);
	}
	...
	public boolean coincident(MotionState other, double epsilon) {
		return MotionUtil.epsilonEquals(t, other.t, epsilon) && MotionUtil.epsilonEquals(pos, other.pos, epsilon)
				&& MotionUtil.epsilonEquals(vel, other.vel, epsilon);
	}
